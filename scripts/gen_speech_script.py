...
schedule_rows = [
    ...
    ["2",     "0:30 – 1:00",   "Agenda · 路线图",              "连宇翔", "0:30"],
    ["3",     "1:00 – 2:30",   "1.0 痛点 + 用户画像",          "连宇翔", "1:30"],
    ["4",     "2:30 – 4:00",   "2.0 两大功能：检索 + 积分",     "连宇翔", "1:30"],
    ["5",     "4:00 – 5:00",   "2.x 技术架构 · 真实部署",       "郁凯杰", "1:00"],
    ["6",     "5:00 – 9:30",   "🎬 LIVE DEMO",                "陈瀚中", "4:30"],
    ["7",     "9:30 – 11:00",  "2.3.4 DFD · 积分系统数据流",    "郁凯杰", "1:30"],
    ["8",     "11:00 – 12:30", "3.0 v2.0 AI 推荐（未来功能）",   "郁凯杰", "1:30"],
    ["9",     "12:30 – 13:30", "4.0 Result & Impact · 项目影响","连宇翔", "1:00"],
    ["10",    "13:30 – 14:30", "Q&A · 问答",                  "全员",   "1:00"],
    ["11",    "14:30 – 15:00", "Thank You · 致谢",            "连宇翔", "0:30"],
]
...
script(doc, "「各位老师、各位同学，下午好。我是连宇翔，旁边是郁凯杰、陈瀚中，我们是这个项目的三人组。」")
...
script(doc, "「凯杰用 1 分钟介绍系统怎么搭起来。然后是中间最大的一段 —— 4 分半钟的 LIVE DEMO，由瀚中现场操作。」")
script(doc, "「之后凯杰再用 3 分钟讲数据流图和 v2.0 AI 推荐。最后我用 1 分钟收 Result & Impact，再留 1 分钟做 Q&A。」")

# --- Slide 3 ---
heading2(doc, "▼ Slide 3：1.0 痛点 + 用户画像  (1:00 – 2:30  ·  连宇翔)")
...
# --- Slide 4 ---
heading2(doc, "▼ Slide 4：2.0 两大功能  (2:30 – 4:00  ·  连宇翔)")
...
script(doc, "「谢谢宇翔。我是郁凯杰，我用 1 分钟讲系统怎么搭起来。」")
...
script(doc, "「关键一句：这个系统不是只在我们电脑上跑的 demo。下一秒交给瀚中做现场演示。」")

# --- Slide 6 ---
heading2(doc, "▼ Slide 6：LIVE DEMO  (5:00 – 9:30  ·  陈瀚中)")
script(doc, "「谢谢凯杰。各位老师、同学，接下来 4 分半钟我带大家做完整演示。」")
script(doc, "「请看屏幕，网站现已打开。注意右下角的『后端在线』指示，说明系统真在跑。」")
script(doc, "")

heading3(doc, "▶ Step 1: 老师当场注册账号 (5:00 – 6:00)")
script(doc, "「先做个最关键的动作 —— 我想请老师当场注册一个账号，证明系统真的能用。」")
script(doc, "「老师可以输一个 8 位学号、随便一个密码、邮箱。」")
script(doc, "「点注册 —— 立刻 100 积分到账。这一秒数据已经写入 MariaDB 的 users 表。」")
script(doc, "[如老师不便，使用预置演示账号 1230000000 / demo123 — 同样有 100 积分]")
...
script(doc, "「点下载，弹出确认框：『要扣 1 分』。这是花积分前必须告知的 UX 原则。」")
script(doc, "「确认 —— 文件真的下到老师电脑，余额从 100 变成 99。这一笔扣分走的是数据库原子事务，等下凯杰会用 DFD 把它讲清楚。」")

heading3(doc, "▶ Step 4: 评分 (7:30 – 8:00)")
script(doc, "「老师给这份资源打 5 星，提交。」")
...
script(doc, "「最后秀一下后台 —— 我现在切到团队管理员账号登录。」")
...
script(doc, "「演示到此结束，把话筒交回给凯杰讲数据流图。」")

# --- Slide 7 ---
heading2(doc, "▼ Slide 7：DFD · 积分系统数据流  (9:30 – 11:00  ·  郁凯杰)")
script(doc, "「谢谢瀚中。刚才大家看到瀚中点了一下下载，老师余额从 100 变成 99。」")
...
# --- Slide 9 ---
heading2(doc, "▼ Slide 9：4.0 Result & Impact  (12:30 – 13:30  ·  连宇翔)")
...
body(doc, "回答（连宇翔）：这是迭代调出来的。最早版本是 50/30/20，文本匹配权重最高。但跑下来发现：标题完全匹配的冷门资源容易抢前列，而真正高质量的资源因为标题用了同义词反而排不上去。")
...
body(doc, "回答（陈瀚中 / 郁凯杰）：是真的，这就是我们一开始让老师注册账号的原因。")
bullet(doc, "数据库：MariaDB 真在云服务器上跑，老师注册的账号已经写入 users 表，演示完可以远程登入服务器看")
